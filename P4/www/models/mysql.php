<?php
    ...
    function getListaActividades($mysqli){
        ...
        if($resultado->num_rows > 0){
            $contador = 0;
            while($row = $resultado->fetch_assoc()){
                $actividades[$contador] = array( 'nombre' => $row['nombre'], 'enlace' => $row['enlace'], 'id' => $row['id']);
                $contador++;
            }
        }
        ...
        if($imagenes->num_rows > 0){
            ...
            $id = 0;
            while($row = $imagenes->fetch_assoc()){
                if( $id != $row['actividad_id']){
                    $id = $row['actividad_id'];
                    $actividades[$contador]['imagen'] = $row['direccion'];
                    $contador++;
                }
            }
        }
    ...
    function editarActividad($id, $nombre, $precio, $fecha, $descripcion, $mysqli){
        $sql = "UPDATE actividades SET nombre = '$nombre', precio = '$precio', fecha = '$fecha', descripcion = '$descripcion' WHERE id = $id";
        if($mysqli->query($sql) === TRUE){
            return true;
        }
        else{
    ...
    function addActividad($nombre, $precio, $fecha, $descripcion, $mysqli){
        
        $sql = "INSERT INTO actividades (nombre, precio, fecha, descripcion) VALUES ('$nombre', '$precio', '$fecha', '$descripcion')";
        if($mysqli->query($sql) === TRUE){
            $id = $mysqli->insert_id;
